<?php 
    ...
    $datos_usuario          = $bodega->datos_usuario($datos_historial_bodega[0]['his_usuario']);
    ...
    $bodega1  = $bodega->datos_sucursales($datos_historial_bodega[0]['his_suc_envia']);
    $destino  = $bodega->datos_sucursales($datos_historial_bodega[0]['his_sucursal']);
        ...
        <div class="col-md-15">
            ...
            <div class="table-responsive p-2">
                <table width="100%" class="table border">
                    <tbody>
                        <tr class="add">
                            <td><b>Suministrado Por:</b><br><?= ucfirst($datos_usuario[0]['us_cli_nombre']) ?></td>
                        ...
                        <tr class="add">
                            <td><b>Bodega Envía:</b></td>
                            <td colspan="2"><b><?= $bodega1[0]['suc_nombre'] ?></b></td>
                        </tr>
                    </tbody>
                </table>
                <table width="100%" class="table border">
                    <tbody>
                        <tr class="add">
                            <td><b>Retirado Por:</b><br><?= ucfirst($datos_historial_bodega[0]['his_retira']) ?></td>
                            <td><b>Rut:</b><br> <?= $datos_historial_bodega[0]['his_rut'] ?></td>
                        </tr>
                        <tr>
                            <td><b>Fecha:</b><br><?= Utilidades::arreglo_fecha2($datos_historial_bodega[0]['his_fecha']) ?></td>
                            <td><b>Hora:</b><br><?= $datos_historial_bodega[0]['his_hora'] ?></td>
                        ...
                        <tr>
                            ...
                            <td><b><?= $destino[0]['suc_nombre'] ?></b></td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
<?php
                        ...
                        for ($i=0; $i < count($datos_historial_bodega); $i++) {
                            ...
                            if($datos_productos[0]['prod_cli_tipo'] == 0){
                                ...
                                $valor_parcial = ($cantidad*$datos_productos[0]['prod_cli_monto']);
                            }else{
                                $cantidad = Utilidades::peso($datos_historial_bodega[$i]['his_cantidad']/1000);
                                $granel   += $datos_historial_bodega[$i]['his_cantidad'];

                                $valor_gramo = ($datos_productos[0]['prod_cli_monto']/1000);

                                $valor_parcial = ($datos_historial_bodega[$i]['his_cantidad']*$valor_gramo);
                            }

                            $sucursal = $bodega->datos_sucursales($datos_historial_bodega[$i]['his_sucursal']);

                            echo '<tr class="content">
                                        <td>'.$datos_productos[0]['prod_cli_codigo'].'</td>
                                        <td>'.$datos_productos[0]['prod_cli_producto'].'</td>
                                        <td>'.$sucursal[0]['suc_nombre'].'</td>
                                        <td>'.$cantidad.'</td>
                                        <td>'.Utilidades::monto($datos_productos[0]['prod_cli_monto']).'</td>
                                        <td>'.Utilidades::monto($valor_parcial).'</td>
                                        <td class="text-left">'.$datos_historial_bodega[$i]['his_comentario'].'</td>
                                    </tr>';
                            $precio_unitario += $datos_productos[0]['prod_cli_monto'];
                            $precio_parcial  += $valor_parcial;
                        }
?>
                    </tbody>
                    <tfoot>
                        <tr class="bg-light">
                            ...
                            <th><?= Utilidades::monto($precio_parcial) ?></th>
                            <th>&nbsp;</th>
                        </tr>
                    </tfoot>
                </table>
                ...
                <div class="products">
                    <table width="100%" class="table border">
                        <tbody>
                            <tr class="add">
                                <td><b>Descripcion Retiro:</b></td>
                                <td><?= $datos_historial_bodega[0]['his_comentario_retira'] ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
